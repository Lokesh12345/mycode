...
$result = $conn->query("SELECT price as price,QUANTITY as quantity,PROD_TYPE,PROD_ID,DATE_DELIVER FROM `daily_delivery` ".$cond." CONS_ID='".$_GET['cid']."' and SUPP_ID='".$_GET['sid']."' order by DATE_DELIVER desc");
...
$productsf=$productsf[$rs["PROD_ID"]];
    if ($outp != "") {$outp .= ",";}
    $outp .= '{"price":"'.$price.'",';
