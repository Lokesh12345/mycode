<?php
include"db.php";
$sql = "SELECT * from daily_transaction where CONS_ID=".$_POST['sid']." and PROD_ID='".$_POST['pid']."'";
...
{
$sql = "UPDATE daily_transaction SET FROM_DATE='{$_POST['fdate']}',TO_DATE='{$_POST['tdate']},DEL_STATUS='NEW',QUANTITY='{$quantity}',trans_type='{$transaction_type}' WHERE FROM_DATE='{$orders['FROM_DATE'][0]}' and TO_DATE='{$orders['TO_DATE'][0]}' and CONS_ID='{$sid}' and PROD_ID='{$pid}'";
				

					if (mysqli_query($conn, $sql)) 
						{
							echo "Record updated 1 successfully";
						} 
						else 
						{
...
// if orginal to date is less than tdate and quanity is greater than orginal quantity and tdate is equal to orginal to date
 else if($orders['TO_DATE'][0]<$tdate && $quantity>$orders['QUANTITY'][0] && $tdate==$orders['TO_DATE'][0])
{
$sql = "UPDATE daily_transaction SET FROM_DATE='{$_POST['fdate']}',TO_DATE='{$_POST['tdate']}',DEL_STATUS='NEW',QUANTITY='{$quantity}',trans_type='{$transaction_type}' WHERE FROM_DATE='{$orders['FROM_DATE'][0]}' and TO_DATE='{$orders['TO_DATE'][0]}' and CONS_ID='{$sid}' and PROD_ID='{$pid}'";
					...
					if (mysqli_query($conn, $sql)) 
						{
							echo "Record updated 2 successfully";
						} 
						else 
						{
							echo "Error updating record: " . mysqli_error($conn);
...
$sql = "UPDATE daily_transaction SET FROM_DATE='{$_POST['fdate']}',TO_DATE='{$_POST['tdate']}',DEL_STATUS='NEW',QUANTITY='{$quantity}',trans_type='{$transaction_type}' WHERE FROM_DATE='{$orders['FROM_DATE'][0]}' and TO_DATE='{$orders['TO_DATE'][0]}' and CONS_ID='{$sid}' and PROD_ID='{$pid}'";
					...
					if (mysqli_query($conn, $sql)) 
						{
							echo "Record updated 3 successfully";
						} 
						else 
						{
							echo "Error updating record: " . mysqli_error($conn);
...
{
$sql = "INSERT INTO daily_transaction (CONS_ID,SUPP_ID,PROD_TYPE,PROD_ID,DAY,FROM_DATE,TO_DATE,DEL_TIME,CRE_DATE,QUANTITY,trans_type,DEL_STATUS)VALUES ('{$orders['CONS_ID'][0]}','{$orders['SUPP_ID'][0]}','{$orders['PROD_TYPE'][0]}','{$orders['PROD_ID'][0]}','{$orders['DAY'][0]}','{$fdate}','{$tdate}','{$orders['DEL_TIME'][0]}','{$CRE_DATE}','{$quantity}','{$transaction_type}','NEW')";
...
$sql = "UPDATE daily_transaction SET FROM_DATE='{$add}',DEL_STATUS='NEW' WHERE FROM_DATE='{$orders['FROM_DATE'][0]}' and TO_DATE='{$orders['TO_DATE'][0]}' and CONS_ID='{$sid}' and PROD_ID='{$pid}' and sno=".$orders['sno'][0];
				

					if (mysqli_query($conn, $sql)) 
						{
							echo "Record updated 4 successfully";
						} 
						else 
						{
							echo "Error updating record: " . mysqli_error($conn);
...
$sql = "UPDATE daily_transaction SET TO_DATE='{$tdate}',trans_type='{$transaction_type}',DEL_STATUS='NEW' WHERE FROM_DATE='{$orders['FROM_DATE'][0]}' and TO_DATE='{$orders['TO_DATE'][0]}' and CONS_ID='{$sid}' and PROD_ID='{$pid}' and sno=".$orders['sno'][0];
...
{
$sql = "UPDATE daily_transaction SET FROM_DATE='{$fdate}',DEL_STATUS='NEW',QUANTITY='{$quantity}' WHERE FROM_DATE='{$orders['FROM_DATE'][0]}' and TO_DATE='{$orders['TO_DATE'][0]}' and CONS_ID='{$sid}' and PROD_ID='{$pid}' and sno=".$orders['sno'][0];
if (mysqli_query($conn, $sql)) 
						{
							echo "Record updated 4 successfully";
						} 
...
if ($ffind=array_search($fdate,$orders['FROM_DATE']))
  ...
  //compare to date
	if ($tfind=array_search($tdate,$orders['TO_DATE']))
		{
			...
			if($ffind==$tfind-1)
				{
					...
					$sql = "UPDATE daily_transaction SET QUANTITY='{$quantity}',trans_type='{$transaction_type}',DEL_STATUS='NEW' WHERE FROM_DATE='{$fdate}' and TO_DATE='{$tdate}' and CONS_ID='{$sid}' and PROD_ID='{$pid}'";
			...
			{
				$sql = "UPDATE daily_transaction SET QUANTITY='{$quantity}',trans_type='{$transaction_type}',TO_DATE='{$tdate}',DEL_STATUS='NEW' WHERE FROM_DATE='{$fdate}' and CONS_ID='{$sid}' and PROD_ID='{$pid}'";
					

					if (mysqli_query($conn, $sql)) 
						{
							echo "to date added successfuly";
						} 
						else 
						{
							echo "Error updating record: " . mysqli_error($conn);
	...
	//if from date dont match to the give from date then else comes
		...
		if ($tfind=array_search($tdate,$orders['TO_DATE']))
		{
		$sql = "UPDATE daily_transaction SET QUANTITY='{$quantity}',trans_type='{$transaction_type}',FROM_DATE='{$fdate}',DEL_STATUS='NEW' WHERE TO_DATE='{$tdate}' and CONS_ID='{$sid}' and PROD_ID='{$pid}'";
					

					if (mysqli_query($conn, $sql)) 
						{
							echo "FROM date added successfuly";
						} 
						else 
						{
							echo "Error updating record: " . mysqli_error($conn);
