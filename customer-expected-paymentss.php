...
$final=0;
$resultse = $conn->query("select PROD_ID,g_id from daily_transaction where CONS_ID='".$cuter."' and DEL_STATUS='Confirm' and trans_type='Orginal'");
while ($rze = $resultse->fetch_array(MYSQLI_ASSOC)) {
$result = $conn->query("SELECT t.sno as sno,t.SUPP_ID as sid,t.DAY as day,t.g_id as gid,t.DAY as ptype,t.trans_type as trans_type,t.QUANTITY as QUANTITY,t.DEL_TIME as DEL_TIME,t.DEL_STATUS as status,t.FROM_DATE as fdate,t.TO_DATE as tdate,t.PROD_ID as pid,p.PROD_NAME as pname,p.IMAGE as image FROM daily_transaction t,products p where t.PROD_ID=p.id and t.CONS_ID='".$cuter."'and t.g_id='".$rze['g_id']."' and t.DEL_STATUS='Confirm' and t.PROD_ID='".$rze['PROD_ID']."' order by t.FROM_DATE asc");
$old="";
$no=0;
$orginal=array();
$from_date=array();
